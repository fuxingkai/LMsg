...
function updateUnReadTotal(tempState) {
  let unreadTotal = 0;
  for (let key in tempState.unreadInfo) {
    unreadTotal = unreadTotal + tempState.unreadInfo[key];
  }
  if (unreadTotal > 0) {
    wx.setTabBarBadge({
      index: 2,
...
function onAddTeamMembers(msg, msgKey, tempState) { // team, accounts, members
  ...
  obj.members.map(item => {
    ...
    if (index === -1 && !listMap[item.account]) {
      ...
      tempState.personList[item.account] = item
    } else {
      list[index] = Object.assign({}, list[index], item)
      listMap[item.account] = list[index]
      tempState.personList[item.account] = list[index]
    }
  })
