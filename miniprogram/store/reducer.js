...
import {
  objectIsEqual,
  setStorageOpts
} from '../utils/sh_util.js'
let app = getApp()
let indexReducer = (state = INITIAL_STATE, action) => {
  if (objectIsEqual(state.rawMessageList,{})){
    let rawMsgs = null;
    let unreadInfo = null;
    try {
      rawMsgs = wx.getStorageSync('rawMsgs');
      unreadInfo = wx.getStorageSync('unreadInfo');
    } catch (e) {
      consonl.log(e);
    }
    if (rawMsgs != null && rawMsgs != "") {
      state.rawMessageList = rawMsgs;
    }
    if (unreadInfo != null && unreadInfo != "") {
  ...
  switch (action.type) {
    ...
    // 当前会话楼盘
    case 'CurrentSessionHomeInfo_Change': {
      let temp = Object.assign({}, state)
      temp['sessionHomeInfo'] = action.payload
      return Object.assign({}, state, temp)
    }
    ...
    case 'RawMessageList_Add_Msg': {
      ...
      */
      ...
      try {
        wx.setStorageSync('rawMsgs', tempState.rawMessageList);
        if (msg.custom != "" && msg.flow == "in") {
          let customContent = JSON.parse(msg['custom']);
          if (customContent != null && customContent.fromNick != undefined) {
            let opt = {};
            opt.photoUrl = customContent.fromHeadImg;
            opt.nickName = customContent.fromNick;
            opt.nimAccid = msg.from;
            setStorageOpts(opt);
          }
        } 
    ...
    case 'UnreadInfo_update': {
      ...
      dealGroupMsg.updateUnReadTotal(tempState);
      let unreadInfo = Object.assign({}, state.unreadInfo, tempState.unreadInfo);
      try {
        wx.setStorageSync('unreadInfo', unreadInfo);
      } catch (e) {
        console.log(e);
      }
      return Object.assign({}, state, tempState)
    ...
    case 'SessionUnreadInfo_update': {
      ...
      sessions.map(item => {
       ...
       dealGroupMsg.updateUnReadTotal(tempState);
      try {
        wx.setStorageSync('unreadInfo', Object.assign({}, state.unreadInfo, tempState.unreadInfo));
      } catch (e) {
        console.log(e);
      }
      return Object.assign({}, state, tempState)
